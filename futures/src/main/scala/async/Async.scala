...
object Async extends AsyncInterface:
  ...
  def concurrentComputations[A, B](
    ...
    val comp1 = makeAsyncComputation1.apply()
    val comp2 = makeAsyncComputation2.apply()
    comp1 zip comp2
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = 
    ...
    def insistRec(counter: Int): Future[A] = 
      val result = makeAsyncComputation.apply()
      result.recoverWith{
        case e: Exception  if counter < maxAttempts => insistRec(counter + 1)
      }
