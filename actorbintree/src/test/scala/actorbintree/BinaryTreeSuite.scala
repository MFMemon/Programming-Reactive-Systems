        ...
        requester.within(5.seconds) {
          val repliesUnsorted = for i <- 1 to ops.size yield try
            ...
            case ex: Throwable if ops.size > 10 =>
              // ops.take(i).foreach(op => op match
              //   case Insert(req, id, elem) => println(s"${ops.indexOf(op)}: Insert(${elem})")
              //   case Contains(req, id, elem) => println(s"${ops.indexOf(op)}: Contains(${elem})")
              //   case Remove(req, id, elem) => println(s"${ops.indexOf(op)}: Remove(${elem})")
              //   )
